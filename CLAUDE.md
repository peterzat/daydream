...
- **Opt-in Claude-vision aesthetic gate (`daydream/testing/vision_gate.py`).** Rates a render against the WHIMSY rubric via one Opus vision call, returning `{score, passed, reason, banned}` (a disqualifying element caps to FAIL). OFF unless `DAYDREAM_CLAUDE_VISION_GATE` is set (then needs `ANTHROPIC_API_KEY`); design-time/test-time ONLY, never a runtime call (consistent with the local-only generation policy). The `tier_long` probe `tests/drift/test_image_claude_vision.py` skips when off; `bin/game review` annotates each render with the verdict when on, turning the aesthetic eyeball into a machine pass/fail.
