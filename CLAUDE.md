...
**`bin/game world reset --yes`** is the one-command destructive runbook the gate points at: `down` → wipe `live.db*` + the world's image cache → `world load worlds/bunny.json --force` → `up`. Run it after editing `worlds/bunny.json`, or whenever the boot gate blocks.
