...
- **No API key anywhere — including dev tooling and tests — and the agent IS the design-time critic.** There is no `ANTHROPIC_API_KEY` / `OPENAI_API_KEY` in the runtime OR the tooling, and we don't add one. "Opus at design time" means THIS Claude Code agent doing the work in-session: authoring content, reasoning over the world, and reviewing generated artifacts by Reading them directly. The agent has vision, so it grades rendered images against `WHIMSY.md` itself — that IS the aesthetic gate, no cloud call. If a feature or a test looks like it wants a cloud vision/LLM call (an image-quality gate, an LLM-critic pass), that is the signal to make it an in-session agent task, NOT to add a key; a `litellm`/cloud call in tooling is the same anti-pattern as one in the runtime. (This is why the env-gated Claude-vision gate was removed 2026-07-01: it needed a key; the agent does the looking instead.) When a genuine HUMAN eye is wanted, escalate two cheap ways rather than reaching for a key: `qpeek` (`~/src/qpeek/`, driven by `bin/game test human`) shows rendered images to the operator for a rating, or just ask the operator to look in-game in the browser.
...
**Agent automation policy for `bin/game up` / `down` (dev mode — enshrined 2026-07-01).** In dev, which is where this project lives today, the agent may `up` / `down` / `deploy` and take the GPU whenever tests or development work need it. Just grab it — no asking first. If a render, a `bin/game test long`, or a `bin/game review` pass wants the card, stop the server, do the work, and bring it back up. The one courtesy: a one-sentence note when you cycle it ("stopping the server to free the GPU for the forge render"), because a live browser session sees a brief disconnect (the calm "the dream is sleeping…" overlay, which recovers on its own). That's the whole cost, and in dev we don't care. This is a deliberate dev-mode stance so the agent isn't blocked on GPU work; a shared **preview/prod** deployment restores the ask-first, `git push`-category treatment (there the running server is someone else's live session). Default to leaving the server up when you're done unless there's a reason not to.

## Versioning & deploy staleness

Two staleness axes are surfaced/enforced so a stale process or world can't silently mislead (the "I thought we'd already redeployed" trap):

- **Code freshness (process vs HEAD).** The server captures its git short SHA at boot (`daydream/version.py:build_sha`; `DAYDREAM_BUILD_SHA` override; `"unknown"` off-git) and serves it at `GET /status/build`. `bin/game status` and the `bin/game up` already-up message compare it to the working tree's HEAD and print "N commits behind; restart to deploy". Every `state_snapshot` also carries the build; an open tab that sees it change (a redeploy under it, since a WS reconnect never reloads page JS — the cause of the forge id-garbage) auto-reloads ONCE into fresh assets (`web/assets/main.js`, behind a `sessionStorage` loop-guard). Asset URLs are `?v=<sha>`-stamped and `/assets/*` is `no-store` (`daydream/api/nocache.py`); together these close the stale-tab class.
- **World/content version (DB vs code).** `daydream/version.py:WORLD_VERSION` is a `MAJOR.MINOR` constant stamped into each world DB at `world load` (the `worlds.world_version` column, migration 012; the unused integer `schema_version` is a different thing). The server lifespan calls `version.check_world_compat` after `init_live`: a MAJOR mismatch REFUSES boot (the live world can't be carried forward) naming `bin/game world reset`; a MINOR or NULL (legacy) mismatch only warns. The gate is in the lifespan, NOT `db.init_live` — admin tools and the test suite call `init_live`, and blocking there would brick the very `world reset` that fixes a mismatch.

**Bump discipline.** Bump `WORLD_VERSION` MAJOR for a change that makes existing worlds unloadable (a non-additive migration, an objects-model change); MINOR for authored content/behaviour an old world won't reflect (a seed, an NPC prompt, `worlds/clockmakers-loft.json`, a new verb); neither for code-only changes (a redeploy/restart already covers those). (Last bumped to 1.4 for the v1.0 loft batch — propagation config, first-planting close, per-NPC drift pools, 2026-07-07.)

**APP_VERSION (the code's own release version).** `daydream/version.py:APP_VERSION` is the semver the RELEASE is called (1.0.0 as of the v1.0 turn), served as the `app:` line on `GET /status/build` and shown by `bin/game status`. It is pinned to `pyproject.toml`'s `version` by a drift-guard test (`tests/test_version.py`); bump both together at a release cut. Distinct from `WORLD_VERSION` (world CONTENT compat) and from the build SHA (which commit the process runs).

**`bin/game deploy`** is the everyday "make the running server match the code" verb: it restarts FastAPI on the current on-disk code (`down` then `up`) and PRESERVES the live world. Run it after committing/editing code you want live; the build-staleness line above is what flags a deploy is due. It briefly drops connected sessions, but a redeploy-aware client auto-reloads itself into the new build. An agent treats `deploy` as a shared-state action in the same category as `up`/`down` (it drops sessions): auto-run it only when the user asked to deploy/refresh or intent is clearly that, with a one-line note; otherwise ask.

**`bin/game world reset --yes`** is the one-command destructive runbook the gate points at: `down` → wipe `live.db*` + the world's image cache → `world load worlds/clockmakers-loft.json --force` → `up`. Run it after editing `worlds/clockmakers-loft.json` (the canonical world), or whenever the boot gate blocks.
...
- **Room descriptions on entry.** Every `state_snapshot` carries `room.description`; the SPA renders it into the persistent `#room-desc` header. On first entry to a room (when the replayed event log would be empty) the SPA ALSO synthesizes a look-style arrival line (`You are in X. You see: ...`) into the chat, so the log is never blank until you type `look` (client-only, `web/assets/main.js`; `look` is per-viewer, so the server never broadcasts it). Per-connection visit memory makes the header description the FULL stored `rooms.description_cached` on the first entry to a room this session and a short "you return to ..." line on re-entry (sticky for the visit so effect re-snapshots don't shrink it). Pre-baked stored text, never a live LLM call.
...
- **Leave the dream / picker-first entry.** `POST /api/session/leave` rests this session's toon (`toons.release_session_toon`) and marks the session `left`. ANY session with no controllable toon — a fresh connect, after leaving, or a kicked/deleted toon — gets a `{kind: "needs_toon"}` frame on WS connect and the SPA shows the picker (picker-first entry, SPEC 2026-06-30). There is NO default-toon fallback: the legacy `t-wren` fallback (which silently resolved a phantom in a `world load`ed world and no-op'd every input) is removed; `_resolve_controlled_toon_id` returns None and the WS routes to the picker. Create / claim clear `left`.
- **Toon delete.** `POST /api/slots/{slot}/delete` (`toons.delete_slot`) removes a toon and frees the slot — distinct from kick (rest). It DROPS the toon's carried items into its current room (they persist in the world to be found, not destroyed with the toon) and clears its memories; its events stay as append-only history.
...
- **One object store.** Migration 011 unified `rooms`/`toons`/`items` into a single `objects` table (`kind` in `room`/`toon`/`thing`/`prototype`). Containment is a self-referential `location_id` (a toon's location is its room; a thing's is the room or the carrying toon; a room is top-level). Inheritance is `prototype_id` → a `kind='prototype'` row whose `properties.verbs` are the default verb set. Promoted columns are only the toon auth/slot fields (`slot`, `controller_session`, `is_human_controlled`, `kicked_at`) + a partial unique index `(world_id, slot) WHERE kind='toon'`; everything kind-specific (seed, title, exits, appearance, mood, examined_text, ...) lives in `properties_json`. `daydream/objects.py` is the single read/write surface (`get`/`contents`/`in_scope`/`move`/`get+set_property`/`spawn`/`verbs_for`); `rooms.py`/`toons.py` are thin typed views over it, preserving their old APIs (`items.py` was removed in the v1.0 cleanup — use `objects.contents(room_id, kind="thing")`). No other module issues raw `objects` SQL.

- **Closed verbs + MOO dispatch.** `daydream/verbs.py` is a closed, engine-implemented verb set (`look`, `examine`, `take`, `drop`, `talk`, `give`, `use`, `open`, `read`, `plant`, `say`, `go`, `inventory`), each with an arg-spec (needs a dobj? an iobj? valid target kinds for each? per-verb effect allowlist? on the UI verb bar?). `execute_command(actor, verb, dobj_id?, iobj_id?, args?, dobj_name?)` is the single executor: it validates the dobj is in scope and the verb applies (`verb in objects.verbs_for(dobj)` — this rejects "take a toon" / "talk to a rock"), then dispatches by MOO priority (player → room → dobj → iobj). The one object-bound handler in v1 is an NPC's `talk` dialogue on the dobj, which beats the generic stub. Deterministic verbs make NO LLM call; only `talk` (dialogue) and a lazy-cache `examine` (below) do. `inventory` (no dobj) lists carried things. A named-but-out-of-scope target ("take the moon") is passed through as `dobj_name` so the executor reads "you don't see the moon here", distinct from the no-target "Take what?". The SPA mirrors verb-applicability client-side (staging a verb dims objects it can't target, so Talk never prompts on a non-toon), labels the scene WHO YOU ARE / HERE WITH YOU / ON THE GROUND / YOU'RE CARRYING, and never shows object/toon ids in player-visible text (SPEC 2026-06-30).

- **Two-object verbs + object state (SPEC 2026-07-01).** `give` (dobj `thing` → iobj `toon`) and `use` (dobj `thing` → iobj `thing`) declare `needs_iobj` + `valid_iobj_kinds`; `execute_command` has an iobj gate symmetric to the dobj gate (missing/out-of-scope → "…to whom?/on what?", wrong-kind → refused, no mutation). `give` reparents a wanted gift onto the NPC, shifts mood, and spawns the NPC's authored `gives` reward into inventory (deduped by provenance). Object state is free-form `properties.state` (no migration), mutated only through the existing `set_property` effect: `open` is state-gated (`locked` refuses with authored `locked_text`; `unlocked`/`closed` → `open`, narrates `open_text`, reveals the `contains` payload once), `use` reads the target's authored `use` rule `{with, from_state, to_state, text}`, `read` narrates a readable's authored `text`, and `examine` appends `state_text[state]` to the physical seed. Both producers drive the two-object verbs: the parser fast-path grounds "give X to Y" / "use X on|with Y" (`parser._two_target_fast_path`), and the SPA does two-step click staging (stage verb → click dobj → click a kind-valid iobj). The canonical quest lives in `worlds/clockmakers-loft.json`; the deterministic golden playthrough (`tests/test_quest_playthrough.py`) is the durable regression guard.
...
- **World-mutation effect API.** `daydream/skills/effects.py` is the allowlisted vocabulary ALL runtime mutation flows through: `narrate`, `set_property`, `spawn_object`, `move_object`, `spawn_room`, `link_exit`, `rename_object` (with `add_item`/`set_mood` retained as aliases for existing data-skill author files). `dispatch_effects(..., allowed=<subset>)` enforces a per-verb allowlist — a kind outside it is rejected like an unknown kind (no mutation). The RESTRICTED kinds (`spawn_room`/`link_exit` world-shaping + `rename_object` housekeeping) are per-verb OPT-IN only: `allowed=None` (the data-skill default) resolves to `DEFAULT_KINDS`, which excludes them, so an NPC dialogue or standalone data skill can never world-build or rename; `plant` is their sole consumer. `destroy_object` is BUILT (the Zork turn): rule-only kind; contents drop to the holder's location; combat's death path is its main consumer.

- **Dreamseeds / world growth (SPEC 2026-07-02).** The quest-earned dreamseed in the clock case is the growth permission: `plant` it with a short vision phrase and `daydream/growth.py` runs gates (carried → unspent → has `properties.growth` → cap `DAYDREAM_GROWTH_MAX_ROOMS` (default 12, counts only grown rooms) → free compass direction → phrase ≤120 chars + banlist; empty phrase narrates the seed's authored question) → ONE local-LLM call (the dream-gardener prompt: seed boundaries + exemplars, no ids/directions ever shown to the model) → strict validation (schema windows, 0–2 objects reject-not-truncate, WHIMSY banlist, anti-copy vs the seed's exemplars, refusal escape hatch) → a SYNCHRONOUS commit block that re-checks every raced gate, then dispatches spawn_room → link_exit → spawn_object(s) → seed consumption (state `spent`, husk examine, verbs cleared, moved into the grown room) → payoff narrate naming direction + title. Every failure path preserves the seed and mutates nothing. Grown rooms are first-class (provenance `generated_by="plant:<seed-id>"` + structured `grown`; lazy watercolor art on first entry; walkable exits both ways). The authored growth boundaries live in `worlds/clockmakers-loft.json` (the design-time pre-bake: theme/palette/motifs/exemplars/skeletons validated fail-loudly by the loader); the real-GPU composition probe is `tests/drift/test_growth_compose.py` (tier_long), whose golden ratification records the mitigation-ladder rung. **Propagation (SPEC 2026-07-07): the growing-world loop.** A seed whose `growth.propagation` block (chance in (0,1], max_generation 1-4, optional seed_text; loader-validated) survives can, on a successful plant, spawn a fresh plantable dreamseed inside the newly grown room — same commit batch, FIRST among the spawns (so the generative dedup can't eat it), boundaries inherited verbatim with `growth.generation`+1, provenance `propagation:<parent-seed-id>`. The roll is `worldstate.rng(world_id, "seed-propagation:<new-room-id>")` — deterministic; suppressed at generation >= max_generation and at the grown-room cap counting the plant's own room; a malformed runtime block means no propagation, never an exception; zero new LLM surface. **First-planting chapter close:** `growth.first_planting_text` (optional, loader-validated) narrates once when the world's FIRST room grows and lands in the planter's journal via `journal.append_authored`; a worldstate `first_bloom` marker is the re-fire belt. Explicitly not a `win`.
...
- **NPC dialogue binding.** `talk`'s dialogue is resolved by `verbs._bound_dialogue_skill`: the NPC's `properties.dialogue` (a data-skill name) first, else the legacy `t-<name>` convention (seeded Rook/Iris). `load_world` installs per-NPC dialogue from the envelope's `toons[].dialogue` as a hidden data skill (a sentinel `context_predicate` keeps it out of room-affordance lists; `talk` reaches it directly) and sets `properties.dialogue`. The canonical reset world is authored at `worlds/clockmakers-loft.json` (The Clockmaker's Loft: Tace/Bell/Mott); `worlds/bunny.json` (Rook/Iris/Bram) is retained in-repo as the `tests/test_scenario.py` loader fixture (old-style envelope → backward-compat coverage).

## The Zork platform layer (v0.6.0)

Zork I ships as pure world data (`worlds/zork1.json`) on Zork-agnostic engine primitives (SPEC 2026-07-02, 16 criteria). The durable operational facts:

- **Engine purity is a test.** `tests/test_no_world_literals.py` (tier_short) fails any `daydream/**` or `web/assets/**` file containing a Zork literal (word-bounded; worlds/, tests/, docs exempt). Engine primitives carry generic names; ALL Zork behavior is authored data. When adding engine text, expect this gate to read it.
- **World sources are region files.** `worlds/zork1/world.json` (config/verbs/flags/rules/fuses/daemons/scoring/voice) + `worlds/zork1/regions/*.json` assemble via `tools/assemble_world.py` into the committed `worlds/zork1.json` + `oracle_map.json` (our-id ↔ original-name, for the differential oracle). The committed envelope must byte-match a re-assembly (`--check` is a tier_short gate). Authoring sugar: `treasure: {take, case}`, `room.bonus`, `zork_name`. Edit sources, re-assemble, never hand-edit the envelope.
- **The canonical walkthrough is the contract.** `tests/data/zork1_walkthrough.json` (~380 commands, 24 segments) replays as cumulative prefixes in `tests/test_zork_walkthrough.py` (tier_medium) under a zero-LLM spy, ending at exactly 350/Master Adventurer/Stone Barrow. THE REPLAY RUNS AS A PLAYER (`is_human_controlled=1` in the fixture): the wanderer's pickpocket stream only exists for player toons, so an unclaimed replay would diverge from the live game. Combat and daemon rolls key on `(rng_seed, turn, purpose)` where turn = command index − 1; the thief fight is aligned to safe counter-roll turns, so INSERTING OR REMOVING A COMMAND ANYWHERE EARLIER SHIFTS EVERY LATER ROLL — expect to re-derive fight filler and re-probe if the dataset changes upstream of a fight.
- **World ops.** `bin/game world reset --yes --world worlds/zork1.json` makes Zork the live world (clockmakers stays the no-arg default). `bin/game world load worlds/zork1.json --output <side.db> && bin/game world swap <side.db>` is the no-downtime path. `tools/ws_playthrough.py` drives the full walkthrough over WS against the live server (login → claim slot 1 → replay; paced by acknowledgment; `--verify` stops at the first divergence) and writes a narration transcript under `~/data/daydream/rehearsals/`.
- **The differential oracle is optional by design (R8).** `tests/drift/test_zork_oracle.py` (tier_long) replays the walkthrough in dfrotz and compares room/score/inventory at segment ends; combat compares on outcomes via `attack_until_dead`. It skips with a named reason until the operator runs `bin/zork-oracle-bootstrap` (builds the dumb-frotz target; the sandboxed agent cannot compile external code) and places a story file at `~/data/zork/zork1.z3` + exports `DAYDREAM_ZORK_ORACLE_STORY`. Story files never enter git. Design-time ground truth (ZIL source, parsed skeleton) lives at `~/data/zork/`.
- **The retell layer** (`daydream/retell.py`, criterion 13) rides `rules.dispatch` (async for this one hook): a retell-enabled world's rule narrations may be LLM-rephrased into its `voice` register, ONLY on repeat tellings (the scoped rung: the authored line always speaks first; a per-text seen counter in worldstate). Strict validation (proper nouns outside sentence-initial position, digit runs, length ratio, banlist) with the authored text as unconditional fallback; `"verbatim": true` on a narrate effect is absolute, and `eligible()` walls off mechanical shapes (short lines, quoted speech, shaped text, shouting). `DAYDREAM_RETELL_ENABLED=0` is the kill-switch; tests force it off so criterion 1's zero-call spy stays honest. The drift probe (`tests/drift/test_retell_probe.py`) fingerprints the second telling at temperature 0.
- **WORLD_VERSION note.** The zork envelope was authored at 1.3; the constant is now 1.4 (the v1.0 loft batch), so a live zork world warns-and-boots (MINOR) and a fresh `world load` stamps 1.4.

## The v1.0 features (SPEC 2026-07-07)

The release turn's four flagships and their operational facts:

- **Toon portraits** (`daydream/images/client.py:portrait_target`/`cached_portrait_url`, `daydream/images/workflows/painterly_portrait.json`). A toon with a non-empty `appearance_seed` lazily gets a 640x768 watercolor portrait through the SAME persistent-image pipeline as room art (`target_kind='toon'`, cached, `generated_assets`-recorded, exclusive arbiter slot). The WS layer enqueues co-located unpainted faces on connect and controlled moves (dedup shared with room art), emits `toon_image_ready` into the toon's current room, and re-snapshots on it (it's in `_EFFECT_MUTATION_KINDS`) so faces appear live; toon cards (self included) carry cached-only `image_url`, and `GET /api/slots` carries cached-only `portrait_url` -- **reads never trigger renders; the picker never paints**. `PersistentTarget.workflow_name` selects the per-kind workflow; the default preserves every room cache key byte-for-byte (regression-tested). The framing clause is `PORTRAIT_FRAMING_CLAUSE`, mirrored in WHIMSY.md "Portrait prompt suffix" with a drift test (A/B-ratified 2026-07-07: head-and-shoulders over figure-vignette). ComfyUI-down degrades to the SPA's placeholder without blocking play.
- **Dream journal** (`daydream/journal.py`). `POST /api/session/leave` fires `journal.write_entry(toon_id)` as a fire-and-forget task: ONE local-LLM call (purpose `journal`, temp 0) over the toon's own events (`events.fetch_for_toon`: actor or recipient rows; room-broadcast NPC replies stay unattributed -- documented limitation), validated (refusal parse, 60-500 chars, WHIMSY banlist), appended to `properties.journal` (FIFO cap 10) via `set_property` directly (no event -- journals are private). Sequence idempotency: `journal_last_seq` advances ONLY on success, so leaving twice with no new events writes nothing and a skipped entry stays tellable. The leave endpoint always succeeds regardless of LLM state; WS disconnect is deliberately NOT a trigger. Snapshots carry the CONTROLLED toon's journal only (never a co-located player's); the SPA shows a once-per-toon-entry "previously, in your dream" beat and renders the satchel's collection page from it. `_event_lines` frames narrates as "you saw: ..." -- the 7B otherwise folds a named NPC's deeds into "you" (journal probe, 2026-07-07). `DAYDREAM_JOURNAL_ENABLED` is the kill switch (default on; conftest forces 0; quality probe `tests/drift/test_journal_probe.py`).
- **Endings** (`effects.py:_apply_win` + the SPA). `game_won` is world-scoped (`room_id=None` passes the WS room filter to every connected player) and carries score + rank frozen at the win moment; `worldstate.status_block` gains `won` so late joiners/reconnects derive a quiet reopenable end-marker from any snapshot. The SPA presents a dismissible "The End" storybook page; the world keeps running. All ending text is world-agnostic (the no-literals gate reads web/assets).
- **Onboarding** (`web/index.html` + `main.js`). A static "How to Dream" book spread (speaking, verbs/objects, exits, satchel, leaving/picking a toon) auto-shows once per browser (`localStorage['dd-help-seen']`) at the picker's arrival beat, layered above the picker (z-70), reopenable anytime from the footer `?`, dismissed by one click. Pure client.
- **Keepsakes are real.** `_object_card` carries `detail` (lazy-cached `examined_text`, fallback authored seed -- pre-baked text, never a live call); keepsake cards caption with it, and the satchel's right page renders the journal instead of decorative-empty slots.
- **Benign refusals: resolved.** `tests/drift/test_dialogue_refusal_probe.py` (tier_long) attributes every dialogue fallback to its layer; 2026-07-07 measured 0/21 on live loft greetings. `tests/security/test_benign_refusals.py` pins the deterministic halves. See BACKLOG's closed entry for the verdict.
...
The qualitative checks (image aesthetics, NPC voice) and the connection-overlay visual are batched and, where possible, made mechanical so a turn spends eyes-on time on gameplay, not on "does it render / read right". Three layers:

- **`bin/game review` — one offline contact sheet.** Loads `worlds/bunny.json` into a throwaway temp DB (no live server, no `live.db` touch, no API key) and writes ONE self-contained `index.html` under `~/data/daydream/reviews/<ts>/`: every aesthetic anchor render (incl. the forge), a fresh `talk` sample per authored NPC (Rook/Iris/Bram, from the shipped dialogue prompts), and a short browser-checklist for the one irreducible live glance (the connection overlay). The operator reviews everything in a single glance instead of a live reset + a browser session per check. Needs ComfyUI + vLLM up for the renders; either down degrades that section to a note. Per the GPU policy, run with the game DOWN (`daydream/review.py`).
- **Perceptual + voice regression proxies (ratify-once, then mechanical).** Image: `tests/drift/aesthetics/*.json` are dHash anchors compared to `tests/baselines/image_*.golden.json` under `tier_long` — the `forge` anchor is the authored `r-forge` seed verbatim, so a forge-render regression fails by design. Voice: `tests/test_voice_baseline.py` (`tier_short`) globs `docs/pretty/voice-samples/*.md`, classified by a `<!-- baseline-class: ... -->` marker (`tracked` / `regression-demo` / `documented-failure`), and asserts distinct narrate openers, so a new tracked baseline auto-extends the regression with no code edit.
- **Agent aesthetic review (no API key).** The aesthetic critic is THIS Claude Code agent. After `bin/game review` renders the anchors, the agent Reads each PNG and grades it against `WHIMSY.md` (cozy soft watercolor; the forge's anvil/bellows actually legible; no banned moods), recording the verdict in-session. No `litellm`, no `ANTHROPIC_API_KEY`, no runtime dependency (see the generation policy). When a human eye is genuinely wanted, escalate to `qpeek` (`bin/game test human`) or ask the operator to look in-game. (An earlier env-gated litellm vision gate was removed 2026-07-01 for needing a key; the agent does the looking instead.)
