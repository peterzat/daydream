...
- **World-mutation effect API.** `daydream/skills/effects.py` is the allowlisted vocabulary ALL runtime mutation flows through: `narrate`, `set_property`, `spawn_object`, `move_object`, `spawn_room`, `link_exit` (with `add_item`/`set_mood` retained as aliases for existing data-skill author files). `dispatch_effects(..., allowed=<subset>)` enforces a per-verb allowlist — a kind outside it is rejected like an unknown kind (no mutation). The world-shaping kinds (`spawn_room`, `link_exit`, SPEC 2026-07-02) are per-verb OPT-IN only: `allowed=None` (the data-skill default) resolves to `DEFAULT_KINDS`, which excludes them, so an NPC dialogue or standalone data skill can never world-build; `plant` is their sole consumer. `destroy_object` remains documented, not built.
