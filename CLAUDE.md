...
`bin/game up`, `bin/game down`, `bin/game status`, `bin/game logs` are the supported daydream-server entry points; `bin/game comfyui-up`/`down` and `bin/game vllm-up`/`down` manage the inference engines (see "External engines" below); `bin/game image-test "<prompt>"` is the aesthetic A/B harness for image gen. `bin/game up-all` is the one-shot bundled boot — starts FastAPI synchronously, then kicks off vLLM and ComfyUI in the background (engines warm over ~30-60 s; the game is reachable as soon as FastAPI's readiness poll completes). There is no symmetric `down-all`; `bin/game down` stops only FastAPI, so engines started by `up-all` need explicit `vllm-down` / `comfyui-down`. Each daemon command writes a PID file to `$XDG_RUNTIME_DIR/daydream-<env>/<process>.pid` and a log alongside; `bin/game status` reports liveness and reachability for all three processes plus the current `DAYDREAM_ACCESS` mode and a UFW-reminder warning when `public`.
