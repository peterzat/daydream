...
**What happened.** NPC presence narration shipped 5/5 across two increments: migration 007 adds a nullable `toons.presence_text` column (Rook's set to "Rook is at the bellows, sleeves a little sooty, humming something slow under their breath."; Wren's stays NULL); the `Toon` dataclass gained the field with a safe accessor so pre-007 DBs still load. The WS broadcast loop now calls `_emit_npc_presence_narrates` in its controlled-move branch — AFTER the post-move snapshot, filtering out the controlled toon and empty/NULL/whitespace-only greetings. The emitter is NOT wired into the effect-mutation snapshot refresh (data skills dispatching in a populated room don't re-greet) nor the initial connect path (reconnect would duplicate history). Three forge E2E tests now consume Rook's narrate between the post-move snapshot and the forge dispatch; the happy-path test's narrate-text assertion doubles as the criterion 3 regression guard (asserts "embers" / "ring", NOT "bellows", so a leaked Rook narrate on mutation refresh would fail it). Two new WS tests: move-into-forge fires the narrate in order; initial-connect-to-meadow fires no stray narrate ahead of a user-initiated `say`. Short 255, short+medium 335.

**Questions and directions.**
- *`say to <npc>` + reactive NPC response* — the real interaction layer. Rook can be seen, examined, and greets you when you arrive; next step is making him audibly present. Extend `say` to accept `to <target>` or add a core `say_to` skill; pair with an authored data skill that gives the NPC a voice bound to their seed. Directly unblocks `voice-samples-capture` and `qwen-2.5-7b-rp-ink-trial` (both gate on "real NPC dialogue"). Scope is substantive (LLM-in-the-loop + prompt design), but the safety baseline already covers the surfaces this would add.
- *Testing debt is aging*. `gameplay-scenario-tests` and `security-tests-tier` have been revisit-candidates for two turns running. Grouped as a modest "testing tier bump" turn they'd plug regression surfaces before drift. Consider scheduling before another feature slice to pay down debt.
- *`toon-slot-management`*. The 5-slot picker UI + `kicked_at`-promotes-to-NPC path. Still defensible as the scaffolding for multi-human play, but adds UI + backend work without an immediate gameplay payoff; lowest urgency of the three.
- *Keeper image moment (not a spec)*. With Rook now at the forge and the image-gen pipeline live, this is a good turn-close opportunity to run `bin/game image-test` + `pretty` to capture a Rook-at-the-forge shot for `docs/pretty/`. Not infra work; flagging in case you want to grab it before context moves on.

**Revisit candidates** (BACKLOG sweep; criteria now plausibly hold):
- `gameplay-scenario-tests` — two consecutive turns of scripted multi-step WS flows now exist (`test_ws_forge.py` + the new presence-narrate tests); formalizing more is overdue.
- `security-tests-tier` — still not picked up two turns after `data-skills-cli` + `safety-baseline-v1` landed; banlist / refusal / tag-wrap surfaces remain concrete regression targets.
- `toon-slot-management` — "first NPC authored" half of the revisit criteria triggered last turn; still applicable.

<!-- SPEC_META: {"date":"2026-04-24","title":"NPC presence narration","criteria_total":5,"criteria_met":5} -->
