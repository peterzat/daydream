...
**vLLM GGUF integration is blocked on a Python packaging bug, not an architecture or VRAM issue.** The MN-12b-RP-Ink Q4_K_M GGUF (~7 GB resident) was downloaded successfully (`bartowski/MN-12b-RP-Ink-GGUF/MN-12b-RP-Ink-Q4_K_M.gguf` lives in `~/.cache/huggingface/hub/` and would fit under the 0.45 mem-util ceiling). vLLM 0.19.1's startup path crashes during `engine_args.create_engine_config` → `maybe_override_with_speculators` → `transformers.configuration_utils.PretrainedConfig.get_config_dict` → `transformers.modeling_gguf_pytorch_utils.load_gguf_checkpoint` → `is_gguf_available()` → `version.parse('N/A')` → `packaging.version.InvalidVersion: Invalid version: 'N/A'`.

Root cause: `transformers.utils.import_utils._is_package_available('gguf', return_version=True)` calls `importlib.metadata.packages_distributions()` to find the distribution name for the import name "gguf"; the result does not contain a "gguf" key (verified directly), so the lookup raises `KeyError`, falls into the except branch which tries `getattr(gguf, '__version__', 'N/A')`, and the `gguf` module does not expose `__version__` (verified directly), yielding "N/A". That string is then passed to `version.parse(...)` and explodes.

This was tested at gguf 0.18.0 (the bootstrapped version) and at gguf 0.17.1 (downgraded for diagnosis); both reproduce. Multiple resolution paths exist but each requires a deliberate user decision: (a) install a known-good `gguf` version that registers correctly in `packages_distributions()` (need to research which versions ship valid metadata); (b) patch the installed `gguf/__init__.py` in the vLLM venv to add `__version__` (works but lives outside the project tree and gets wiped by `bin/vllm-bootstrap`); (c) extend `bin/vllm-bootstrap` to install a working gguf version after the main install; (d) switch GGUF inference to llama.cpp (out of spec scope). No path is taken in this turn — criterion 2 of this spec explicitly anticipated a vLLM blocker and routed it here.

The AWQ vLLM workflow is unaffected: `bin/game vllm-up` with the default Qwen 2.5 7B AWQ model loads cleanly post-experiment (verified). The gguf downgrade left in the venv is harmless for AWQ usage; the next `bin/vllm-bootstrap` re-resolves whatever pip pulls.

C1 (tic-detection probe) shipped independently and is the durable gain from this turn — any future template tic regression now surfaces as a `tier_short` failure. C5 (tests stay green) holds.
...
<!-- SPEC_META: {"date":"2026-05-06","title":"RP-Ink A/B + tic-detection probe","criteria_total":5,"criteria_met":2} -->
