...
- [x] **The live world is reset onto the new schema.** The existing w-bunny world was archived (insurance: `archives/w-bunny-20260630-033905.tar.gz`), re-authored on the object/verb schema at `worlds/bunny.json`, loaded keyless, and installed as the live world (`worlds-dev/live.db`); the game boots on it and Rook's `talk` dialogue spawns the canonical "sheaf of papers". Verified live against the RUNNING server via a WS E2E (10/10): meadow snapshot with the verb bar + clickable lantern + entity sidecar, `examine` echoes the seed (no LLM), walk to the forge, `talk` to Rook → the real local Qwen emitted `spawn_object` and the papers appeared. The one-time visual browser eyeball (aesthetics) is the operator's remaining glance.
...
<!-- SPEC_META: {"date":"2026-06-30","title":"objects + local LLMs: a MOO-style object/verb core with natural-language input","criteria_total":19,"criteria_met":19} -->

<!-- 19/19. The object/verb/parser/UI/generative core landed as committable, test-green increments (short + medium tiers pass; a tier_long parser-grounding probe ran live against vLLM and grounded all cases correctly). The live world reset was performed: w-bunny archived for insurance, re-authored at worlds/bunny.json, installed as worlds-dev/live.db, and the running server verified end to end over a real WS connection (10/10: snapshot shape, examine, walk, Rook spawning the papers via the real local LLM). Two content bugs in the reset world were found by actually running it and fixed (Wren is_human_controlled 1->0 so it is claimable; bare item names so verb wording reads "the brass lantern" not "the a brass lantern"). The only thing not done by the agent is a human's visual aesthetic glance in a browser. (Original meta said criteria_total 18; the actual count is 19.) -->
