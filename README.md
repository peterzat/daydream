...
Latest stable cut: **v0.6.0** (Zork I on daydream — the platform turn: the complete Great Underground Empire hosted as a swappable world of pure DATA on new Zork-agnostic engine primitives; the canonical 350-point walkthrough replays deterministically with zero LLM calls, and the same commands complete live over WebSocket). Runs on a single Linux dev box (RTX 4000 SFF Ada, 20 GB VRAM); designed to port to Cloudflare and containers later. Test gates: the fast and integration tiers (see [Tests](#tests)) are both 100% green; real-GPU drift + parser-grounding + retell + growth-composition probes run on-demand under `bin/game test long`.
