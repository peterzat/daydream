...
- **One-line description:** Add `bin/install-hooks` that writes `.git/hooks/pre-commit` (running `bin/game test short`) and `.git/hooks/pre-push` (running `bin/game test medium`), idempotent, with a `--uninstall` flag. Today `.git/hooks/` holds only the `.sample` files; a Claude Code session running `bin/game test short` before a commit is the only effective gate, and that depends on the agent remembering. The installer turns the gate into a mechanical pre-commit invariant on this single-contributor box.
- **Why deferred:** Existing single-contributor cadence (every commit goes through a Claude Code session that already runs `bin/game test short`) means the gate is honored in practice. The installer earns its keep when (a) commits start landing without a Claude Code session in the loop, or (b) an actual regression slips past because the agent forgot to run tests. Distinct from the deferred `ci-pipeline` entry, which is about CI for a second contributor — pre-commit hooks are for the local single-contributor tighten-the-loop case.
- **Revisit criteria:** First commit lands on main without `bin/game test short` having been run in the same session (a regression slips); OR a second contributor joins and local hooks become the bridge before CI lands.
