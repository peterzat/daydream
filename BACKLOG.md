...
- **Status (2026-05-06, blocked on vLLM GGUF integration):** Active in spec 2026-05-06; the spec adopted re-pick path (b) — Mistral Nemo 12B via GGUF Q4_K_M. `huggingface_hub` confirmed `allura-org/MN-12b-RP-Ink` is 12.25B BF16 = 24.5 GB (the prior status note's "~7 GB bf16" was wrong; that figure refers only to Q4 GGUF). No AWQ/GPTQ variants exist; only GGUF re-quantizations. Downloaded `bartowski/MN-12b-RP-Ink-GGUF/MN-12b-RP-Ink-Q4_K_M.gguf` (6.96 GB). vLLM 0.19.1 startup crashes during config load: `transformers.utils.import_utils._is_package_available('gguf')` cannot resolve a version because (i) `importlib.metadata.packages_distributions()` does not register the gguf import name and (ii) the `gguf` module exposes no `__version__` attr — fallback returns "N/A", which `version.parse` rejects with `InvalidVersion`. Reproduced at gguf 0.18.0 and 0.17.1. Resolution paths (none taken in 2026-05-06): (i) install a known-good gguf version, (ii) patch `gguf/__init__.py` post-bootstrap (gets wiped on re-bootstrap), (iii) extend `bin/vllm-bootstrap` to inject the patch, (iv) switch GGUF inference to llama.cpp. SPEC 2026-05-06 criteria 2-4 marked blocked; criteria 1 (tic-detection probe) and 5 (tests green) shipped. The vLLM AWQ workflow is unaffected; downgraded gguf left in venv is harmless for AWQ.
