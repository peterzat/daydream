...
- **Status (2026-04-24, blocked on model selection):** Voice-bench harness + AWQ baseline landed (SPEC 2026-04-24 C1+C2). Verified via `huggingface_hub.HfApi().model_info('Qwen/Qwen2.5-7B-RP-Ink')` — the named repo does NOT exist (RepositoryNotFoundError). `HfApi().list_models(search='rp-ink')` found: `allura-org/MN-12b-RP-Ink` (Mistral Nemo 12B base, ~7 GB bf16, GGUF Q4 ~4 GB); `allura-org/Qwen2.5-32b-RP-Ink` (32B — too big per localreview Stage 3 rejection); `estrogen/teleut-7b-rpink-*-adpt` (7B LoRA adapters on non-Qwen base, requires vLLM LoRA machinery). None satisfy "drop-in Qwen 2.5 7B swap." Re-picking requires (a) finding a real Qwen 2.5 7B creative-writing finetune that exists today, (b) accepting a different base model and re-scoping the A/B to "Qwen vs. Nemo voice" (loses the same-base controlled-variable benefit), or (c) shipping a LoRA-adapter flow + serving the 7B-teleut base. Pick the path before re-attempting this entry.
- **Status (2026-05-06, gguf bug worked around; finetune NOT pipeline-fit):** Active in spec 2026-05-06; the spec adopted re-pick path (b) — Mistral Nemo 12B via GGUF Q4_K_M. `huggingface_hub` confirmed `allura-org/MN-12b-RP-Ink` is 12.25B BF16 = 24.5 GB (the prior status note's "~7 GB bf16" was wrong; that figure refers only to Q4 GGUF). No AWQ/GPTQ variants exist on HF; only GGUF re-quantizations. Downloaded `bartowski/MN-12b-RP-Ink-GGUF/MN-12b-RP-Ink-Q4_K_M.gguf` (6.96 GB). vLLM 0.19.1 startup initially crashed at `transformers.is_gguf_available()` because `importlib.metadata.packages_distributions()` does not register the gguf import name AND the gguf module exposes no `__version__` attr; reproduced at gguf 0.17.0, 0.17.1, 0.18.0, 0.19.0 (every version in vLLM's supported `>=0.17.0` range). Resolution: extended `bin/vllm-bootstrap` with an idempotent post-install step that appends `__version__ = "<version>"` to the installed `gguf/__init__.py` (durable across re-bootstrap; remove the patch block whenever upstream fixes packaging metadata). With the patch in place, vLLM serves the GGUF at `--max-model-len 4096` (down from 8192; the 12B Q4 weights leave ~1.11 GiB for KV cache vs the 1.25 GiB 8192 needs; 7248 is vLLM's reported ceiling). However, the finetune does not produce usable output through daydream's data-skill pipeline: under the strict-JSON `response_format` constraint, raw model output is `{"effects":[{}]}` (JSON-valid but content-empty); without the constraint, output is verbose free-form roleplay continuation that overshoots the "1-2 sentences" format. MN-12b-RP-Ink is trained for prose continuation; its structured-output capability is degraded relative to the base Mistral Nemo Instruct. Voice-quality A/B is moot for this candidate. Captured at `docs/pretty/voice-samples/2026-05-06-mn-12b-rp-ink-q4_k_m.md` with a `## Verdict` section explaining the degenerate output. A meaningful next-turn A/B requires either (i) a creative-writing finetune that preserves JSON-following capability, (ii) a daydream pipeline change that accepts free-form prose and post-parses, or (iii) a fairer base-vs-finetune A/B (Mistral Nemo Instruct vs. Mistral Nemo + RP-Ink) factoring out the base-architecture axis.
